...
pub mod slab;
/// b-tree which stores its data in a reference to [shareable_slab::Store], so its store can
/// be shared with other b-trees
#[cfg(any(doc, feature = "shareable-slab"))]
pub mod shareable_slab;
/// b-tree which stores its data in a reference to [concurrent_shareable_slab::Store], which
...
/// [shareable_slab_simultaneous_mutation::Store], which is faster and allows concurrent
